\documentclass[title-style=modern,plantuml=true,print-ndn=false,auto-generate=false,bib-file=content/literature.bib]{udhbwvst}

\dhbwSetup{%
    author          = Adrian Stengle und Simon E\ss{linger},
    faculty         = Wirtschaft,
    field of study  = Wirtschaftsinformatik,
    academic year   = 2017,
    course          = B,
    title           = Projekt Maschinenbau,
    subtitle        = Softwarekonzept,
    text type       = Seminararbeit,
    company name    = Gruppe 1 \cdot\ Animal Abduction Crew,
    lecturer        = Prof.\ Dr.\ Wolfgang Funk
}

\usepackage{booktabs} % pretty tables

\input{content/acronyms.tex}

\begin{document}

\dhbwSetFrontMatter

\dhbwPrintTitle
\dhbwPrintNonDisclosureNotice
\dhbwPrintTableOfContents

\dhbwSetListMatter

\dhbwPrintListOfAcronyms
\dhbwPrintListOfFigures
\dhbwPrintListOfTables

\dhbwSetMainMatter

\input{content/text/01_Aufgabenstellung.tex}
\input{content/text/02_Vorgaben.tex}
\input{content/text/03_Konzept.tex}
\input{content/text/04_Aufbau_des_Roboters.tex}
\input{content/text/05_Usecases.tex}
\input{content/text/06_Anforderungen.tex}
\input{content/text/07_Objekterkennung.tex}
\input{content/text/08_Gesamtablaufdiagram.tex}

\begin{dhbwappendices}
\input{content/text/04_Aufbau_des_Roboters_appendices.tex}
\input{content/text/07_Objekterkennung_appendices.tex}
\end{dhbwappendices}

\dhbwPrintBibliography

\end{document}
